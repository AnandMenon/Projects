...
public class OnTestClickListener implements OnClickListener {
	...
	private class Task extends AsyncTask<Void, String, Void> {
		...
		{
			try {
				String fetchKey,fetchValue,fetchOwner;
		...
		private boolean testDelete() {
			try {
				for (int i = 10; i < TEST_CNT; i++) {
					String where = "key" + Integer.toString(i);
		...
		private boolean testQuery() {
			try {
				for (int i = 0; i < TEST_CNT; i++) {
					...
					String owner = (String) mContentValues[i].get(OWNER_FIELD);
